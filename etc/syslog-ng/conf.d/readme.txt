# Files in conf.d are processed by syslog-ng in alphabetic order, first files beginning with uppercase characters, 
# then files beginning with lowercase characters
...
#send Ignition Server logs that match { match("^catId=" value("PROGRAM")) and match("^msgId=" value("MESSAGE")) and match("ADDomainAsset=" #value("MESSAGE")); };
